...
server<-function(input,output,session) {
  ...
  out_x<-eventReactive(input$goButton, {
    ...
    data <- readWorksheet(loadWorkbook(inFile$datapath,create=F),original_data_sheet_name)
    ########################## ALR-fin
    ...
    if(length(run_per_district)>0){
      ...
      stop("Choose at least one district")
    }
    
    if(outbreak_threshold > 1| outbreak_threshold <= 0) {
      stop("You specified an invalid value for the threshold variable. 
           The threshold variable should be in (0,1].")
    }
    
    if(!spline %in% c(TRUE,FALSE)){
      stop("You specified an invalid value for the spline. 
           The spline option should takes value either 0(No) or 1(Yes).")
    }
    ...
    data <- data %>% group_by(district_no) %>%
      mutate(outbreak_moving_tmp_sum=rollapply(outbreak_tmp,FUN=sum,width=list(-3:3),
                                               align="center",fill=NA,na.rm=T,partial=T),
    ...
    data <- data %>% 
      group_by(district_no, week) %>% 
      mutate(outbreak_moving=mean(outbreak_moving_tmp,na.rm=T),
    ...
    if(generating_surveillance_workbook==FALSE){
      ...
      for(j in 1:length(run_per_district)){
        ...
        ggsave(filename=paste('Runin_period_for_district(',run_per_district[j],')_',rand_n,'-',
                              format(Sys.Date(),"%d%b%y"),'.png',sep=''),width=20,device="png",
               height=12,units='cm',dpi=300)
        
        img_nam <- paste('Runin_period_for_district(',run_per_district[j],')_',rand_n,'-',
    ...
    if(generating_surveillance_workbook==FALSE){
      ...
      get_eval_res <- function(d){
        rstats <- data1 %>% filter(district==run_per_district[d]) %>% 
          summarise(n_weeks=sum(weeks),
        ...
        rstats1 <- rstats %>% select(n_weeks,n_outbreak_weeks,n_outbreak_periods,n_outbreak_mean_cutoff,n_alarms,
                                     n_correct_alarms,n_false_alarms,n_missed_outbreaks,n_no_alarm_no_outbreak,all_cases,
                                     n_cases_below_threshold)
    ...
    if(generating_surveillance_workbook==TRUE){
      ...
      for(j in 1:n_cols){
        ...
        if(spline==FALSE){
          ...
          a <- c("District :" ,"Alarm indicator :","z_outbreak :","Prediction distance:","Outbreak window size:","Alarm window size:","outbreak_threshold:", "alarm_threshold:","Outbreak week length:")
          a1 <- c("District:" ,"Alarm indicators:","z_outbreak:","Prediction distance:","Outbreak window:","Alarm window:","outbreak_threshold:", "alarm_threshold:","Outbreak week length:")
    ...
    # Copy the excel files to www folder --------------------------------------
